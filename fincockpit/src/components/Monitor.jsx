...
let getData = async () => {
    let info = []; messagePop('loading', 'Loading data, please wait!');
    ...
    try {
        ...
        let recommendation = responseRecommendation.data[0];

        let securityData = Object.assign(profile, recommendation); console.log(securityData);
        if (Object.keys(securityData).length > 0) {securityData.key = i; info = [...info, securityData]};
    }
    catch (error) {console.log(error);};
    };
    setData(info); messagePop('success', 'Data retrieved!');
};
...
const columns = [
    ...
    {title: 'WatchList', dataIndex: 'action', key: 'watchlist', align: 'center',
        render: (_, record) => (<Button size='small' onClick={() => {addToWatchlist(userId, record.ticker, record.name); messagePop('info', `Adding ${record.name} in Watchlist!`)}}>Add</Button>)}
];
